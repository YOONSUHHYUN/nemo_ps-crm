...
<script>
      ...
      if (xhr.readyState === XMLHttpRequest.DONE) {
        if (xhr.status === 200) {
          ...
          for (var i = startIndex; i < endIndex; i++) {
            ...
            <a href="${row['고유키'] ? 'update_customer.php?num=' + encodeURIComponent(row['고유키']) : 'update_target.php?num=' + encodeURIComponent(row['num'])}" class="num-link">${row['고유키'] ? '기가입' : row['num']}</a>`;
  ...
  fetchTargetData();
  setInterval(fetchTargetData, 50000); // Refresh every 5 seconds (adjust as needed)
...
<div>
  ...
  <h1><center>Platform Sales CRM ..ETC..</center></h1>

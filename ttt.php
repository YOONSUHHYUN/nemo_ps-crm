...
<html>
  ...
  <title>신규,복귀 업체 일반금액</title>
  </head>
  <body>
    <h1><center>신규업체 상품금액</center></h1>

  <h3><td colspan="3"><?php echo $_GET['address1']." ".$address; ?></td> 상품금액</h3>
<h3>기간 : <?php
$today = date("Y-m-d");
echo $today;
?> ~ <?php echo $row11['enddate']; ?> (<?php echo $row8['usedays']; ?>일) </h3>
    ...
    <table class="table table-bordered">
      <thead>
        <tr>
           ...
           <th scope="col" style='text-align: center; vertical-align : middle;'>총 <?php echo $row8['usedays']; ?>일 사용금액</th>
        </tr>
      </thead>
      ...
      <thead>
        <tr>
            ...
            <td colspan="1" style='text-align: center; vertical-align : middle;'><?php echo "<font color=red>"; echo number_format($row10['newnormalprice']); ?>원</p></td>
        </tr>
      </thead>
    </table>
        ...
        <table class="table table-bordered">
          <thead>
            <tr>
               ...
               <th scope="col" style='text-align: center; vertical-align : middle;'>할인율</th>
               <th scope="col" style='text-align: center; vertical-align : middle;'>단품(상가OR사무실) <?php echo $row8['usedays']; ?>일 사용금액</th>
               <th scope="col" style='text-align: center; vertical-align : middle;'>패키지(상가+사무실) <?php echo $row8['usedays']; ?>일 사용금액</th>
          ...
          $primeumprice = "SELECT * FROM `product_premium` WHERE `시` = '서울특별시' AND `구` = '강남구' AND `구분` IN ($codes)";

      ...
      <tr>
        ...
        <td>
              ...
              if (isset($_POST['submit'])) {
                  ...
                  // Prepare the SQL query
                  $queryjung1 = "SELECT COUNT(DISTINCT num) AS contact_count
                            FROM agent
                            WHERE `담당자`='정종형' AND `수신`<>'문자' AND DATE_FORMAT(`컨택일`, '%Y-%m-%d') = '$selected_date'";

                  // Execute the query
                  $resultjung1 = $conn->query($queryjung1);

                  if ($resultjung1->num_rows > 0) {
                      // Output data of the first row
                      $rowjung1 = $resultjung1->fetch_assoc();
                      $contact_countjung1 = $rowjung1['contact_count'];

                      echo " $contact_countjung1 ->";
                  } else {
                      echo "No contacts found on $selected_date";
                  }
              }

...
#기준금액
$cp = "SELECT (SELECT 가격 FROM product_nomal WHERE 시 = '".$_GET['address1']."' and 구 = '".$_GET['address2']."' limit 1 )/10 * '".$number."' as cp";
...
<html>
  ...
  <body>
        ...
        <div class="row">
          <div class="col-xs-12">
            <!-- 반응형 이미지 -->
            <img src="banner/bg_img01.jpg" class="img-responsive img-full-width" alt="배너 이미지">
          </div>
        </div>
      </div>
...
<?php echo $row100['지역']; ?>
  ...
  -->
          ...
          $primeumprice = "SELECT * FROM `product_premium` WHERE `시` = '{$_GET['address1']}' AND `구` = '{$_GET['address2']}' AND `구분` IN ($codes)";
...
#            $row101 = mysqli_fetch_assoc($result101);
            ...
            #패키지 계산-----
                        ...
                        <thead>
                            <tr>
                                ...
                                <td colspan='1' style='text-align: center; vertical-align: middle;'>{$row100['사무실']}</td>
                                <td colspan='1' style='text-align: center; vertical-align: middle;'>{$row100['상가사무실']}</td>
                                <td colspan='1' style='text-align: center; vertical-align: middle;'>
                                    <button onclick="addToCart('{$row100['지역']}', '{$row100['사무실']}', '사무실')">Add to Cart</button>
                                </td>
                                <td colspan='1' style='text-align: center; vertical-align: middle;'>
                                    <button onclick="addToCart('{$row100['지역']}', '{$row100['상가사무실']}', '상가사무실')">Add to Cart</button>
                                </td>
                            </tr>
                        </thead>
                        HTML;
      ...
      <script>
        function addToCart(region, office, type) {
            // Create a new div element
            var newDiv = document.createElement('div');

            // Set the inner HTML of the new div
            newDiv.innerHTML = "<thead><td>" + region + "</td><td>" + office + "</td><td>" + type + "</td></thead>";
...
<script>
    ...
    function addToCart(value) {
        ...
        // You can perform additional actions here, like updating the cart display.
    }
</script>
